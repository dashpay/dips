...
Each Asset Lock transaction must create an unspendable output by using an OP_RETURN as the first script. The following criteria apply to the OP_RETURN output:
...
* It must be the only output in the transaction using an OP_RETURN
...
There are two ways to prove transaction finality: either by including the InstantSend lock of a recent transaction or by referencing the chain height at which a transaction was included in a ChainLocked block. Typically, it is preferable to use the InstantSend lock for proof, but a ChainLock can be used if an InstantSend lock is not available.

## Asset Unlocking

Withdrawing locked credit pool funds back to Dash is done by creating a Core chain unlock transaction signed by an LLMQ_100_67 quorum. This transaction will release Dash from the credit pool while reducing its balance by an equivalent amount.
...
An active LLMQ_100_67 quorum must sign Asset Unlock transactions per the process described in [DIP-7](./dip-0007.md). The signing session’s requestID is calculated as follows:

    requestID = SHA256(SHA256("plwdtx", index))

For reference, the following table shows some example index values and their request IDs:
