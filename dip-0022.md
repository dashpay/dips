...
Multi-Quorum ChainLocks are relayed through the P2P network via a `CLSIGMQ`
...
To ensure the convergence of block signing we reuire honest quorums to form a
...
the first one must wait for a `CLSIGMQ` message from the previous quorum before
...
there is no `CLSIGMQ` from the previous quorum after some time, it's recommended
to look for a `CLSIGMQ` from a quorum before the previous one and so on. When
there is no `CLSIGMQ` from any previous quorum after some extended timeout, the
...
for its LLMQ, it must create a `CLSIGMQ` P2P message and propagate it to non-SPV
...
If a block has been received locally and no `CLSIGMQ` message has been received
...
applied. When the `CLSIGMQ` message for this (or another) block is later
...
then a new `CLSIGMQ` message with an aggregated `sig` and a combined `signers`
...
The first seen aggregated `CLSIGMQ` for a known block is the one that should be
used to lock the chain.

## Conflicting signed blocks

It is possible for a malicious quorum to double-sign two different blocks
and to produce two valid conflicting recovered signatures. `CLSIGMQ` messages
