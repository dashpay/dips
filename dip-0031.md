...
4. Validate the signedHeight is within [TipHeight - 576, TipHeight]
   * Penalty: small P2P penalty (1) if signedHeight is inside the range [TipHeight - 576 - 5, TipHeight + 5], it is possible we think we are synced, when we are not, or blocks are coming quickly and a previously valid message is no longer valid.
