...
We consider the scenario where an attacker has assumed control of a number of masternodes. We will then calculate two probabilities that the attacker will
be able to disrupt the network. Specifically, we will calculate the probability
that an attacker will be able to prevent all ChainLocks for a twelve hour period, and
we will calculate the probability that an attacker can successfully create a malicious
or erroneous ChainLock.
...
160 out of 400 masternodes in the quorum.  If the attacker controls
`m` out of the `N` total masternodes we can compute the
number of outcomes that satisfy the event "exactly 160
nodes of the quorum are controlled by the masternode."
Specifically there are
<sub>`160`</sub>`C`<sub>`m`</sub> * <sub>`240`</sub>`C`<sub>N - m</sub>
outcomes where the attacker controls exactly `160` masternodes.
Thus, the probability that the attacker controls exactly `160` nodes is
<center>
![Threshold Case](./dip-0008/ThreshCase.gif).
</center>

The attacker could also be successful if they control <b>more</b> that `160`
...
| N | m |Type of Attack|success probability in one trial |
|---|---|--------------|--------------------------------|
|5000|500| withhold |  2.79e-64|
|5000|1000| withhold | 5.09e-22 |
|5000|1500| withhold | 5.59e-6 |
|5000|500| malicious | 7.11e-157 |
|5000|1000| malicious | 2.86e-76 |
|5000|1500| malicious | 1.89e-38 |
|2000|200 | withhold | 0 |
|2000|400 | withhold | 6.88e26 |
|2000|600 | withhold | 1.12e-6 |
|2000|200 | malicious | 0 |
|2000|400 | malicious | 9.49e-94 |
|2000|600 | malicious | 3.93e-45 |
...
malicious software. Also if node opperators do not check the digital signature
...
download. As long as the correct public key is acquired than it is not
possible for the official download to be spoofed.
