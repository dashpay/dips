...
The request id to sign a block must be calculated deterministically once for
each `blockHeight`.

The message hash is always the hash of the signed block.

The sign hash for the BLS signature is calulated as
...
The request id for each LLMQ is calculated as `SHA256("clsig", blockHeight, step)`
where `step` starts from `0` and increments by `1` starting with the most recent LLMQ.

After a LLMQ member has successfully recovered the final ChainLocks signature
for its LLMQ, it must create a P2P message and propagate it to all nodes.
The message is called `CLSIG`, just like before, but it has a slightly different structure:
...
| signers | byte[] | (signersSize + 7) / 8 | Bitset representing the aggregated signers of the BLS signature |

`signersSize` must be equal to the number of active quorums, `step`th bit in
the `signers` bitset must be set to `1`.

This message is propagated through the inventory system.

## Handling of signed blocks

The node should ensure that the fully singed block is locally accepted as the
...
announcing it to other nodes:
  ...
  a set of active quorums must be calculated.
  2. A request id must be calculated.
  3. A quorum must be selected from the set based on the calculated request id.
...
following verification before announcing it to other nodes:
  ...
  1. Based on the deterministic masternode list at the given height,
  a set of active quorums must be calculated.
  2. A request id must be calculated using the bit in the `signers` bitset set to `1` as a `step`.
  3. A `step`th quorum must be selected from the set.
  4. The BLS signature must verify against the quorum public key and the sign hash.
...
a combined `signers` field should be formed and relayed to other nodes.
...
must perform the following verification before announcing it to other nodes:
  ...
  1. Based on the deterministic masternode list at the given height,
  a set of active quorums must be calculated.
  2. Request ids must be calculated for every bit in the `signers` bitset set to `1` as a `step`.
  3. All `step`th quorums must be selected from the set.
  4. The BLS signature must verify against the set of quorum public keys and the set
  of corresponding sign hashes.
