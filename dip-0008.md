...
In general, all blocks which are considered valid could also be considered for ChainLocks.
There is however a small risk of transactions conflicting with InstantSend locks, which
requires some conflict resolution, which means that ChainLocks need to be prioritized over
InstantSend locks when conflicts arise.

As such conflict resolution is quite severe and in some sense voids all the security gained
by InstantSend, it is desired to reduce the probability of such conflicts to practically zero.

This can be achieved by a change in block template generation. Miners are encouraged to only
include transactions in blocks which are considered "safe". A transaction is considered safe
if it is InstantSend locked or locally known for at least 10 minutes without a conflicting
InstantSend lock appearing in the meantime. The default implementation for block template
generation (as found in Dash Core) will be changed to honor this.

At the same time, masternodes should only try to sign/lock blocks which include only "safe"
transactions. This in turn means that ChainLocks will only be created when the whole block
is considered "safe", which means that the probability of a conflicting InstantSend lock
...
get InstantSend locked in a retroactive way when these were unknown before appearing in a
mined block. This prevents blocks which contain unlocked transaction to suppress ChainLocks,
as it allows the ChainLocks system to sign a block retroactively as well.

## Signing attempts

Each masternode should periodically try to sign the current chain tip. When the tip already got a valid
...
ChainLock is deeper then 6 blocks.

When these checks pass, each masternode must invoke the [DIP007
