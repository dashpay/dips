...
* `type_byte`: `0xb0` for P2PKH or `0xb8` for P2SH
...
* The type byte is either `0xb0` or `0xb8`.
...
* `<data-part>` contains:
  * one type byte (`0xb0` P2PKH, `0xb8` P2SH), followed by
...
| Platform P2SH  | `0xb8`    | `h`                  |

The type bytes `0xb0` and `0xb8` were specifically chosen so that the first character after the `1` separator is always `k` for P2PKH and `h` for P2SH. This provides immediate visual identification of address type:

* P2PKH addresses always match pattern `dash1k...` (mainnet) or `tdash1k...` (testnet)
* P2SH addresses always match pattern `dash1h...` (mainnet) or `tdash1h...` (testnet)

This mapping is guaranteed because the bech32 `convertbits()` function deterministically converts the first 5 bits of the type byte to the first data character. The high 5 bits of `0xb0` (binary `10110...`) map to index 22 in the bech32 alphabet (`k`), while the high 5 bits of `0xb8` (binary `10111...`) map to index 23 (`h`).
...
* Hardware wallets MUST validate the HRP to confirm network identity and MUST enforce the type byte (`0xb0` or `0xb8`). Devices MUST display a user-facing descriptor: “Dash Platform address” for P2PKH and “Dash Platform script address” for P2SH.
...
**Note:** The type byte values defined here (`0xb0`, `0xb8`) are specific to the bech32m address encoding. Platform implementations use compact variant indices (e.g., `0x00` for P2PKH, `0x01` for P2SH) for internal storage and database keys. Implementations MUST map between these representations when encoding or decoding addresses.
...
function encode_platform_address(hash160, type, network):
    ...
    type_byte = 0xb0 if type=="p2pkh" else 0xb8 if type=="p2sh" else error()
...
| Vector | Payload (HASH160) | Mainnet (`dash`) | Testnet (`tdash`) |
| ------ | ------------------------------------------ | ------------------------------------------------- | -------------------------------------------------- |
| 1      | `f7da0a2b5cbd4ff6bb2c4d89b67d2f3ffeec0525` | `dash1krma5z3ttj75la4m93xcndna9ullamq9y5e9n5rs` | `tdash1krma5z3ttj75la4m93xcndna9ullamq9y5fzq2j7` |
| 2      | `a5ff0046217fd1c7d238e3e146cc5bfd90832a7e` | `dash1kzjl7qzxy9lar37j8r37z3kvt07epqe20ckxfezw` | `tdash1kzjl7qzxy9lar37j8r37z3kvt07epqe20cxp68nq` |
| 3      | `6d92674fd64472a3dfcfc3ebcfed7382bf699d7b` | `dash1kpkeye606ez89g7lelp7hnldwwpt76va0v3j6x28` | `tdash1kpkeye606ez89g7lelp7hnldwwpt76va0vp4fcmf` |

### P2SH example

Payload: `43fa183cf3fb6e9e7dc62b692aeb4fc8d8045636`

* Mainnet: `dash1hppl5xpu70aka8nacc4kj2htflydspzkxc57vr75`
* Testnet: `tdash1hppl5xpu70aka8nacc4kj2htflydspzkxcyela06`
