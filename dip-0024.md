...
is from the block hash where its DKG session started, forcing each quorum index to start DKG at a
unique height ensures they will all have different quorum hashes. Second, this benefits the network
by limiting the number of quorums in the same resource-intensive DKG phases simultaneously.
