...
| Field | Type | Size | Description |
| ----- | ---- | ---- | ----------- |
| bestCLHeightDiff | varint | 4 | Number of blocks between the current block and the block locked with `bestCLSignature`. |
| bestCLSignature | BLSSig | 96 | Newest ChainLock signature known by the miner. |
| creditPoolBalance | int64_t | 8 | Total amount of locked credits at the block height. |

More information on the fields `bestCLHeightDiff` and `bestCLSignature` can be found in [DIP0029 - Randomness Beacon For LLMQ Selection](https://github.com/dashpay/dips/blob/master/dip-0029.md#change-to-the-coinbase-transaction).
More information on the field `creditPoolBalance` can be found in [DIP0027 - Platform Credit Funding and Withdrawals](https://github.com/dashpay/dips/blob/master/dip-0027.md).
