...
In order to allow old clients to successfully parse the `MNLISTDIFF` message, the new fields `type`, `platformHTTPPort` and `platformNodeID` will be serialised only when client's protocol version is greater or equal than 70227.
