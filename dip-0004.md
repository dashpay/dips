...
Clients with protocol version greater than or equal to 70228 will recieve the following field just before `proRegTxHash`.
...
Starting from the first block after the v19 fork, the `type` field is serialised.
...
| Field               | Type | Size | Description                       |
|---------------------| ---- | ---- |-----------------------------------|
| nVversion             | uint16_t | 2 | Version of the `MNLISTDIFF` reply |

For now, only `nVersion` 1 is used.
