...
Starting from the first block after the v19 fork, the following changes are activated:

1. pubKeyOperator of all SML entries are serialised using the basic BLS scheme (otherwise, the legacy BLS scheme is used.)
2. type field is serialised.

In order to allow old clients to successfully parse the `MNLISTDIFF` message, the new `type`, `platformHTTPPort` and `platformNodeID` fields will be serialised only when the client's protocol version is greater than or equal to 70227.
...
The `version` field indicates which BLS scheme is used to serialise all SML entries' `pubKeyOperator` field of `mnList`.
