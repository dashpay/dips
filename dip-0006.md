...
`qfcommit` with version 2 or 4 include the `quorumIndex` field highlighted below as described in [DIP24](dip-0024-rotation.md#quorum-composition):
...
Starting from v19, `quorumPublicKey` will be serialised using basic BLS scheme.
The `version` field indicates which scheme to use for `quorumPublicKey` serialisation so the existing chain containing keys serialised with the legacy scheme can continue to be synced properly.
