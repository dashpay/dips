...
If a DKG failed or a miner did not receive a final commitment in-time, the quorum commitment inside the block must have the quorumHash set to the correct quorumHash and quorumVvecHash to hash(quorumHash, newTipHeight). Also, the signers and validMembers bitsets should be set to the quorumSize and all bits set to zero. All other fields must be set to the null representation of the field’s types. The special handling of the quorumVvecHash field ensures that all null commitments result in a different transaction hash.
