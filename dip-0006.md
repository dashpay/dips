...
| keepOldConnections | The number of quorums for which we should keep old connections. This should be at least one more then the active quorums set. Used for [intra-quorum communication](#intra-quorum-communication). |
| recoveryMembers | Maximum number of members we try to send all sigShares to before considering a [signing session](https://github.com/dashpay/dips/blob/master/dip-0007.md) failed when [concentrated recovery](https://github.com/dashpay/dash/pull/3389) is active for the quorum |
