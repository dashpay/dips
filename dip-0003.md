...
A ProRegTx or ProUpServTx is invalid if any of these conditions are true (applicable only when type is 1):
  ...
  2. platformP2PPort, platformHTTPPort are not set to default mainnet values.
  3. platformNodeID is null or is a non-hex string.
  4. platformNodeID is not unique in the network.
...
Find previous Masternode payee. If the last payee is a Regular Masternode, go to step #5
If the last HPMN payee was already paid 4 times, go to step #5
If the last HPMN is not present in current MN list anymore, go to step #5
Select last HPMN as payee and increment his consecutive payment state by one. Return

1. Take the last block's payee. If its type is Regular Masternode, go to step 5.
2. If the last HPMN payee was already paid 4 blocks in a row, go to step 5.
3. If the last HPMN is not present in current Masternode list anymore, go to step 5.
